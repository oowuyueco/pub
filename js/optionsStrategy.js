...
function 统计策略(triggerLogObj, 策略名 = "", 高低位 = "低位", 汇总N = 5, 过滤M = 10) {
    ...
    trigDateArr.forEach((ele, index) => {
        let date = ele[0], trig策略s = ele[1], profileN = ele[2], 位箭 = ele[3];
        ...
        // 第一个>=N2&&>20天的,否则默认2。
        if (overMin期权days(profileN, nextSecondDeliveryKey)) { nextN期权到期 = nextSecondDeliveryKey; N = 2 }
        else if (overMin期权days(profileN, nextThirdDeliveryKey)) { nextN期权到期 = nextThirdDeliveryKey; N = 3 }
        else { nextN期权到期 = nextSecondDeliveryKey, N = 2; }
